...
public class WatsonAssistantConfig {
    ...
    @Pattern(regexp = "[a-zA-Z0-9]*", message = "La clave de api tiene caracteres no válidos")
    ...
    @Pattern(regexp = "https:\\/\\/(www\\.)?[-a-zA-Z0-9@:%._\\+~#=]{1,256}\\.[a-zA-Z0-9()]{1,6}\\b([-a-zA-Z0-9()@:%_\\+.~#?&\\/\\/=]*)", message = "La url del servicio contiene errores")
    private String url;

    @NotNull(message = "La fecha de versión es obligatoria")
    @NotBlank(message = "La fecha de versión es obligatoria")
    @Pattern(regexp = "^\\d{4}(-)(((0)[0-9])|((1)[0-2]))(-)([0-2][0-9]|(3)[0-1])$", message = "La fecha de versión contiene errores, debe tener el formato DD-MM-AAAA")
